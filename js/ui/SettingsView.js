...
const INTERRUTTORI_CHE_APRONO = new Set([
  'assistente.enabled',
  'telegram.enabled',
  // Impostandolo sopra zero compare il cursore dell'opacità.
  'scan.screenSaverMin',
  ...
  _scanCard(cfg) {
    return this._card(t('sec.scan'), null, [
      ...
      this._range('scan.screenSaverMin',
        ...
        0, 120, 5, ' min'),
      ...(((cfg.scan?.screenSaverMin || 0) > 0) ? [
        this._range('scan.screenSaverOpacita',
          P(['Quanto scurire', 'How dark']),
          P(['1 è nero pieno. ⚠️ Sotto 1 resta un\'ombra appena percepibile di ciò che c\'è sotto, e serve a chi assiste: dice a colpo d\'occhio che il programma è vivo e non bloccato. Uno schermo completamente nero è indistinguibile da un computer spento o da un programma in errore, e chi entra nella stanza di notte non può saperlo senza toccare nulla.',
             '1 is fully black. Below 1 a faint shadow remains, which tells at a glance that the program is alive and not stuck.']),
          0.5, 1, 0.01),
      ] : []),
      /* ⚠️ Parametri che esistevano ma non si potevano toccare.
       *
       * Una verifica automatica li ha trovati: undici, in scansione e
       * puntatore. Chi assiste non poteva cambiarli e spesso non
       * sapeva nemmeno che esistessero — e alcuni contano molto per
       * chi comanda con un gesto solo. */
      this._range('scan.graceMs',
        P(['Tolleranza sul tempo di reazione', 'Reaction-time tolerance']),
        P(['Selezionando un attimo DOPO che la scansione è passata alla voce successiva, viene presa la precedente — quella che si voleva. ⚠️ Serve a chi seleziona alzando l\'occhio: il movimento va riconosciuto, e in quel frattempo la scansione avanza. Se le selezioni cadono sulla voce dopo quella voluta, alzarlo.',
           'Selecting just AFTER the scan moved on still picks the previous item.']),
        0, 600, 25, ' ms'),
      this._toggle('scan.selectWakes',
        P(['Un gesto di selezione risveglia dalla pausa', 'Select also wakes from pause']),
        P(['Acceso, qualunque gesto riprende la scansione. Spento, serve il gesto di ripresa dedicato: utile per chi ha movimenti involontari frequenti e rischierebbe di ripartire senza volerlo.',
           'When on, any gesture resumes scanning.'])),
      this._toggle('scan.showMenuItem',
        P(['Voce "menu" nel ciclo principale', 'Show the "menu" item'])),
      this._toggle('scan.autoSpeakOnWordEnd',
        P(['Pronuncia da sola a fine parola', 'Speak automatically at word end'])),
      this._range('scan.mediaCount',
        P(['Quanti contenuti per gruppo', 'Items per media group']), null, 3, 30, 1),
      this._range('scan.maxCyclesMedia',
        P(['Giri prima di uscire da un gruppo di contenuti', 'Cycles before leaving a media group']), null, 1, 10, 1),
    ...
    return this._card(t('sec.pointer'),
      ...
      [
        ...
        this._range('pointer.calibrationBordoGiri',
          ...
          0, 4, 1),
        this._range('pointer.calibrationSettleMs',
          P(['Attesa prima di raccogliere', 'Settle before sampling']),
          P(['⚠️ Il tempo che si lascia allo sguardo per ARRIVARE sul bersaglio. Troppo corto e i campioni contengono ancora il tragitto invece della posizione raggiunta: è ciò che faceva rifiutare la calibrazione per "movimento troppo piccolo".',
             'Time allowed for the gaze to REACH the target before sampling.']),
          200, 2000, 50, ' ms'),
        this._range('pointer.minSpan',
          P(['Escursione minima accettata', 'Minimum accepted range']),
          P(['Sotto questa escursione la calibrazione viene rifiutata: un puntatore tarato su un movimento troppo piccolo sarebbe inutilizzabile e frustrante. Abbassarlo solo per chi ha un movimento oculare molto ridotto.',
             'Below this range the calibration is refused.']),
          0.01, 0.30, 0.01),
        this._range('pointer.calibrationBordoOgniMs',
          P(['Campione ogni (giro del bordo)', 'Sample every (border lap)']), null, 80, 1000, 20, ' ms'),
        ...
        this._range('pointer.stripeSpeedMs',
          ...
          600, 8000, 100, ' ms'),
        this._toggle('pointer.stripeContinua',
          P(['Le bande ricominciano invece di spegnersi', 'Stripes restart instead of stopping']),
          P(['⚠️ ACCESO è quasi sempre giusto: chi ha un solo gesto non può riaccendere le bande — il pulsante può premerlo soltanto chi assiste, e se non c\'è nessuno la persona resta senza alcun modo di comandare. Le bande sono il suo cursore, come la voce guida è la sua tastiera.',
             '⚠️ ON is almost always right: someone with a single gesture cannot switch the stripes back on.'])),
        this._range('pointer.stripeMaxSweeps',
          P(['Passate a vuoto prima di ricominciare', 'Empty sweeps before restarting']), null, 1, 10, 1),
